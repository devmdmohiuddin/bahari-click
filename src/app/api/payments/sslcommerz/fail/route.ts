...
async function handle(request: NextRequest) {
  ...
  try {
    if (tranId) await markPaymentFailed(tranId, PaymentStatus.failed);
  } catch (error) {
    console.error("[payments/fail]", error);
  }
  return NextResponse.redirect(`${base}/?payment=failed&tran=${tranId ?? ""}`, { status: 303 });
}
